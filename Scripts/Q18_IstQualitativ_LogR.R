...
logisticModel <- glm(IstQualitativ ~ Gewicht+Hoehe+Durchmesser, family = "binomial", data = logisticTrainData)
...
newData <- data.frame(Gewicht = seq(min(logisticTestData$Gewicht), max(logisticTestData$Gewicht), len=nrow(logisticTestData)),
                      Hoehe = seq(min(logisticTestData$Hoehe), max(logisticTestData$Hoehe), len=nrow(logisticTestData)),
                      Durchmesser = seq(min(logisticTestData$Hoehe), max(logisticTestData$Hoehe), len=nrow(logisticTestData)))
newData$QualPred = predict(logisticModel, newData, type="response")

# Plot der Sigmoid Kurve nur mit Hoehe und Gewicht
plot3d(logisticTestData$Gewicht, logisticTestData$Hoehe, logisticTestData$IstQualitativ, col=orange)
